...
def create_transform_window(settings):
    ...
    layout = [  [sg.Text('Transformation!', font='Any 15')],
                ...
                [TextLabel('Date of last export'),sg.Input(key='-last_date-'), sg.CalendarButton('Choose Date', target='-last_date-', format="%Y%m%d%H%M")],
                
                [sg.Button('Do it!'), sg.Button('Cancel')]  ]

    window = sg.Window('Transformation!', layout, size=(600, 300), keep_on_top=True, finalize=True)
